...
class PeepSoActivityListTable extends PeepSoListTable 
{
	...
	public function get_columns()
	{
		$columns = array(
			...
			'activity_action' => __('Details', 'peepso'),
			'ID' => __('ID (link)', 'peepso'),
	...
	public function get_column_data($item, $column_name)
	{
		...
		case 'post_status':
			...
			if('publish' == $item->post_status) {
				return __('Published','peepso');
			}

			if('archive' == $item->post_status) {
				return __('Archived','peepso');
			}

			if('trash' == $item->post_status) {
				return __('Trash','peepso');
			}

			if('private' == $item->post_status) {
				return __('Private','peepso');
			}
			return (__(ucfirst($item->post_status),'peepso'));
//		case 'post_title':
//			return ('<a href="' . PeepSo::get_page('activity') . '?status/' . $item->$column_name . '/" target="_blank">' . $item->$column_name . '</a>');
		...
		case 'user_avatar':
			...
			$content .= '<img src="' . $user->avatar . '" title="' . $user->get_username() . '" width="48" height="48" alt="" style="float:left;margin-right:3px;"/></a>';
			return ($content);
