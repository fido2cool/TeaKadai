...
//PeepSo::log('  extra: ' . var_export($extra, TRUE));
        ...
        if (NULL !== $repost) {
            ...
            #PeepSoMailQueue::add_message($orig_post->post_author, $data, __('Someone shared your post', 'peepso'), 'share', 'share', PeepSoActivity::MODULE_ID);

            $note->add_notification($owner, $orig_post->post_author, __('Shared', 'peepso'), 'share', self::MODULE_ID, $id);
        }
...
//				PeepSoMailQueue::add($owner_id, $data, __('Someone Liked your post', 'peepso'), 'likepost');
    ...
    public static function set_ban_status(PeepSoAjaxResponse $resp)
    {
        // check admin access
        ...
        $ban_status = $input->post_int('ban_status', 1);

        // don't allow users to un/ban themselves
        if ( PeepSo::get_user_id() === $user_id ) {
            $resp->success(0);
            $resp->error(__('You cannot ban yourself', 'peepso'));
            return;
        }
        ...
        if( $user_id == 0 || NULL == $user->get_id() ) {
            $resp->success(0);
            $resp->error(__('Invalid user', 'peepso'));
            return;
        }
        ...
        if( 0 === $ban_status ) {
            ...
            $resp->set('message', sprintf(__('%s has been unbanned', 'peepso'), $user->get_display_name()));
        } else {
            $user->set_user_role('ban');
            $resp->set('message', sprintf(__('%s has been banned', 'peepso'), $user->get_display_name()));
        }
...
//				'type' => $type,
        ...
        if(in_array($post->post_type, $doreadmore))
        {
            ...
            if ($length > $min_readmore && $min_readmore > 0) {
                ...
                if (!$is_permalink) {
                    ...
                    $readmorelink ='&hellip; <a class="ps-stream-post-more ps-js-content-excerpt-toggle"' . $link . '>' . __('Read more', 'peepso') . '</a>';
    ...
    public function has_posts()
    {
        if (NULL === $this->post_query) {
            ...
            $owner = apply_filters('peepso_user_profile_id', 0);
            $user = PeepSo::get_user_id();
            $this->get_posts(NULL, $owner, $user, 1);
        }
...
//PeepSo::log(__METHOD__.'() - post data: id=' . $this->post_data['ID']); // var_export($this->post_data, TRUE));
    ...
    public function show_posts_per_page(PeepSoAjaxResponse $resp)
    {
        ...
        $this->get_posts(NULL, $owner, $user, $page);
    ...
    public function get_like_status($post_id, $module_id = PeepSoActivity::MODULE_ID)
    {
        ...
        $like_icon = 'thumbs-up';
        $like_label = $like_text = __('Like', 'peepso');
