...
(function($){
	function PsPopoverNotification(elem, options)
	{
		...
		this.fetch = function( callback ) {
			...
			this._notifications = {};
			$PeepSo[ method === 'get' ? 'getJson' : 'postJson' ](this.opts.source, req, function(response) {
				if (response.success) {
	...
	function get_latest_count() {
		if (peepsodata.currentuserid > 0) {
			$PeepSo.disableAuth().disableError().getJson("notificationsajax.get_latest_count", null, function(response) {
	...
	var get_latest_timer = setInterval(get_latest_count, 1000 * 10);
	$(window).on("peepso_auth_required", function() {
		clearInterval(get_latest_timer);
	});

	ps_observer.add_filter("pschat_mark_as_read", function() {
		clearInterval(get_latest_timer);
		get_latest_count();
		get_latest_timer = setInterval(get_latest_count, 1000 * 10);
	}, 10, 2);
