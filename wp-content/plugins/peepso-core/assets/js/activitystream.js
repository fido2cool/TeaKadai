...
{
	if ( this.action_like_progress ) return;
...
PsActivity.prototype.on_commentbox_change = function(textarea)
{
	...
	if ("" !== jQuery.trim($sel.val())) {
		...
		$sel.parents('.cstream-form-input').next('.cstream-form-submit').hide();
...
PsActivity.prototype.show_likes = function(act_id)
{
	...
	if ( this.is_ajax_loading[likes_id] ) {
		this.is_ajax_loading[likes_id].abort();
		this.is_ajax_loading[likes_id] = false;
	}

	this.is_ajax_loading[likes_id] = $PeepSo.getJson("activity.get_like_names", req, function(json) {
