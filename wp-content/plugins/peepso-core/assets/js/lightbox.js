...
PsLightbox.prototype = {
	...
	init: function() {
		if ( ! this.$container ) {
						...
						'<div class="ps-lightbox-wrapper">',
							'<div class="ps-lightbox-content">',
								...
								'<div class="ps-lightbox-imagelink">',
									'<a target="_blank"><i class="icon-resize-full-alt"></i>View full image</a>',
								'</div>',
			...
			this.$imagelink = this.$container.find('.ps-lightbox-imagelink a');
		...
		if ( this.data[ index ] ) {
			...
			if ( this.options.simple ) {
				...
				if ( img.length ) {
					...
					// update link
					src = img.attr('src').replace('/thumbs/', '/').replace( /_l\./, '.');
					this.$imagelink.attr('href', src );
				}
			}
		}
	...
	attachevents: function() {
		...
		this.$imagelink.on('click', $.proxy(function() {
			this.close();
		}, this ));
	},
