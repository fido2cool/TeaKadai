...
class PeepSo
{
    ...
    private function __construct()
    {
        ...
        add_filter( 'pre_get_document_title', array(&$this, 'peepso_change_page_title'));
    ...
    function action_deleted_user( $id )
    {
        ...
        new PeepSoError('after comments: '. var_export($success, TRUE) .' '. $wpdb->last_error,'error','guwno');
	...
	public function peepso_change_page_title($title, $sep=''){
        ...
        if ( !is_admin() ) {
            ...
            if ( isset($post->post_content) && stristr($post->post_content, '[peepso')) {
            ...
            if ($user_id = PeepSo::get_user_id()) {
                ...
                if ('ban' == $user->get_user_role()) {
                    ...
                    echo "<script type=text/javascript>"
                        ." alert('" . __('Your account has been suspended.', 'peepso') . "');"
...
//self::log("adding directory [{$dirname}] to path list");
    ...
    public function enqueue_scripts()
    {
        ...
        wp_register_style('peepso-fontello', PeepSo::get_asset('css/fontello.css'), NULL, PeepSo::PLUGIN_VERSION, 'all');
        wp_enqueue_style('peepso-fontello');
        ...
        $aData = array(
            ...
            'like_text' => _n(' person likes this', ' people like this.', 1, 'peepso'),
            'like_text_plural' => _n(' person likes this', ' people like this.', 2, 'peepso')
        );
...
//		wp_register_script('peepso-login', PeepSo::get_asset('js/login.js'), array('jquery'), PeepSo::PLUGIN_VERSION, TRUE);
        ...
        wp_register_script('peepso-autosize', PeepSo::get_asset('js/autosize.min.js'), array('jquery', 'peepso-underscore'), PeepSo::PLUGIN_VERSION, TRUE);
...
//		wp_enqueue_script('peepso-login');
    ...
    {
        if( 0 === intval(PeepSo::get_option('system_use_peepso_avatars', 0))) {
            return $avatar;
        }
...
//			return ($is_moderator = TRUE);
    ...
    public static function check_permissions($owner, $action, $author, $allow_logged_out = FALSE)
    {
        ...
        if (0 === $owner || (0 === $author && FALSE === $allow_logged_out)) {
            new PeepSoError(__('User id or author id is invalid', 'peepso'));
            return (FALSE);
        }
        ...
        if ($author != self::get_user_id()) {
            new PeepSoError(__('Invalid authorship', 'peepso'));
            return (FALSE);
        }
        ...
        if ($blk->is_user_blocking($owner, $author, TRUE)) {
            ...
            new PeepSoError(sprintf(__('User %1$d tried to write on %2$d\'s wall and was blocked', 'peepso'), $author, $owner));
            return (FALSE);
        }
    ...
    public static function get_user_access($user_id)
    {
        ...
        return $user->get_profile_accessibility();
    }
...
// Versioning
    ...
    public static function check_version_compat($version, $release = NULL, $version_compare = NULL, $release_compare = NULL)
    {
        $version_compare = (NULL === $version_compare) ? self::PLUGIN_VERSION : $version_compare;
        $release_compare = (NULL === $release_compare) ? self::PLUGIN_RELEASE : $release_compare;
        ...
        // explode the parts
        $v_peepso = self::get_version_parts($version_compare);
...
// Admin notices & alerts
    ...
    public static function mailqueue_notice()
    {
        ...
        echo '<div class="error peepso"><strong>' .__('It looks like PeepSo emails were not processing properly. We just tried to fix it automatically.<br> If you see this message repeatedly, consider deactivating and re-activating PeepSo or contacting Support','peepso').'</strong></div>';
