...
class PeepSoForm
{
	...
	public function render_field($field)
	{
		...
		if('message' == $field['type'] ) {
			return '<p style="color:gray;padding-left:10px;padding-right:10px;">'.$field['label'].'</p>';
		}
	...
	public function validate_field(&$field)
	{
		...
		if('recaptcha' == $field['name'] && PeepSo::get_option('site_registration_recaptcha_enable', 0)) {
			...
			$trans = 'peepso_recaptcha_' . PeepSo::get_user_id();
			...
			if( 1 == get_transient($trans)) {
				...
				if (FALSE == $field['valid']) {
					...
					$field['class'] .= ' error';
					$field['error_messages'] = array(__('ReCaptcha security check failed', 'peepso'));
				}
			}
		} else {
