...
//  int $dst_w , int $dst_h , int $src_w , int $src_h )
	...
	public function confirm_avatar(PeepSoAjaxResponse $resp)
	{
		...
		if (FALSE === wp_verify_nonce($input->post('_wpnonce'), 'profile-photo')) {
			...
			if ($input->post('use_gravatar') == 1 && PeepSo::get_option('appearance_use_gravatar') == 1)
			{
	...
	public function get_edit_form_fields()
	{
		...
		$fields = array(
			...
			'gender' => array(
				...
				'value' => $user->profile_fields->get('gender', FALSE),
				'access' => $user->profile_fields->get_acc('gender'),
			),
			'birthdate' => array(
				...
				'type' => 'datepicker',
				'options' => array(),
				'value' => $user->profile_fields->get('birthdate', FALSE), // '1964-02-29',
				'access' => $user->profile_fields->get_acc('birthdate'),
...
//				'required' => 1,
				...
				'type' => 'textarea',
				'raw' => TRUE,
				'value' => $user->profile_fields->get('description', FALSE),
				'access' => $user->profile_fields->get_acc('description'),
			),
			'user_url' => array(
				...
				'class' => '',
				'type' => 'text',
				'value' => $user->profile_fields->get('user_url',FALSE),
				'access' => $user->profile_fields->get_acc('user_url'),
	...
	public function edit_preferences()
	{
		...
		if (0 == PeepSo::get_option('site_profile_posts_override', 1)) {
			unset($fields['peepso_profile_post_acc']);
		}
	...
	{
		$html = '';
		if (get_transient('peepso_'.$this->user_id.'_online')) {
...
//		echo PeepSoViewLog::get_views($this->user_id, PeepSo::MODULE_ID);
	...
	{
		$this->get_wp_user();
		echo htmlspecialchars($this->user->profile_fields->get('description'));
	}
	...
	{
		$this->get_wp_user();
		$dt = $this->user->profile_fields->get('birthdate');
		if (FALSE === $dt)
			return (FALSE);
	...
	{
		$this->get_wp_user();
		$ret = $this->user->profile_fields->get('gender');
		if (FALSE === $ret)
			return (FALSE);
		return (TRUE);
	}
...
//			echo		$aAttr['label'], PHP_EOL;
	...
	{
		$this->get_wp_user();
		$bio = $this->user->profile_fields->get('description');
		if (FALSE === $bio)
			return (FALSE);
		return (!empty($bio));
	}
	...
	{
		$this->get_wp_user();
		$url = $this->user->profile_fields->get('user_url');
		if ($url === FALSE)
			return (FALSE);
		return (!empty($url));
	}
	...
	{
		$this->get_wp_user();
		$ret = '<a href="' . esc_url($this->user->profile_fields->get('user_url')) . '" rel="nofollow" target="_blank">' . $this->user->profile_fields->get('user_url') . '</a>';
		echo ($ret);
	}
