...
class PeepSoLicense
{
    ...
    public static function activate_license($plugin_slug, $plugin_edd)
    {
        ...
        if ( !is_object($validation_data) ) {
            ...
            if (is_wp_error($resp)) {
                ...
                #new PeepSoError("license check finished, $micro_elapsed",'message',$plugin_edd);
                #new PeepSoError($response, 'message', $plugin_edd);
			}
    ...
    public static function check_updates( $plugin_edd, $plugin_version, $file )
    {
        ...
        if(!strlen($version)) {
            ...
            /* LOG */ $start = microtime();
            /* LOG */ #new PeepSoError("GET version.txt from PeepSo.com", 'message', $plugin_edd);

            $version = 0;
            $url = PeepSoRemoteContent::INTEGRATION_JSON_URL.'/version.txt';
            ...
            if (!is_wp_error($resp)) {
                ...
                /* LOG */ #new PeepSoError("FAIL version.txt from PeepSo.com", 'message', $plugin_edd);

            }

        } else {
            /* LOG */ #new PeepSoError("Using transient current version number", 'message', $plugin_edd);
        }

        /* LOG */ #new PeepSoError("Current version number $version", 'message', $plugin_edd);
