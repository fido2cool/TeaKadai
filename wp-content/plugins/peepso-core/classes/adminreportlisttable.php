...
PeepSo::log(__METHOD__."() orderby={$orderby} order={$order}");
	...
	public function get_columns()
	{
		$columns = array(
			...
			'content_summary' => __('Content Summary', 'peepso'),
			'rep_summary' => __('Report Summary', 'peepso'),
			'content_link' => __('Link', 'peepso'),
	...
	public function get_column_data($item, $column_name)
	{
		switch ($column_name)
		{
			...
			case 'rep_summary':
					ob_start(); ?>
					<?php _e('Reports','peepso');?>: <?php echo $item['rep_user_count'];?><br/>
					<?php _e('Reason','peepso');?>: <?php echo $item['rep_reason'];?><br/>
					<?php
					$content = ob_get_contents();
					ob_end_clean();
					return ($content);
