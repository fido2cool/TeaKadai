...
class PeepSoUser
{
	...
	public function __construct($id = 0)
	{
		if ($id > 0) {
			...
			if (count($res) == 0) {
				...
				// @TODO @since 1.6 usermeta birthday, gender and accesibility
			}
			if (count($res) > 0) {
	...
	public function get_coverphoto()
	{
		...
		if (empty($cover)) {
			$gender = $this->profile_fields->get('gender');
	...
	{
		if (get_user_meta($this->id, 'peepso_use_gravatar', TRUE) == 1 && PeepSo::get_option('appearance_use_gravatar') == 1) {
			return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim($this->get_email()))) . '?s=160&r=' . strtolower(get_option('avatar_rating'));
		} 
		...
		if (!file_exists($file)) {
			...
			$g = $this->profile_fields->get('gender');		// gender returns FALSE if gender is inaccessible
			switch ($g)
			{
	...
	{
		$ret = ($this->_get_prop('first_name', $check_acc));
	...
	public function get_display_name()
	{
		...
		$name = apply_filters('peepso_after_get_display_name', $name);
...
//		$this->id = $temp_id;		// we need to keep the *new* user id
		...
		if ( PeepSo::get_option('site_registration_enableverification',0) ) {
			PeepSoMailQueue::add_message($adm_id, $data, __('{sitename} - New User Registration', 'peepso'), 'new_user_registration', 'new_user_registration');
		}
...
//		$dir = WP_CONTENT_URL . '/peepso/users/' . $this->id . '/';
	 ...
	 * Number of Posts on Activity Stream to show at a time.
	 * @return int
	 */
